<div class="our_footer_area">
      <div class="book_now_aera ">
         <div class="container wow fadeInUp">
            <div class="row book_now">
               <div class="col-md-6">
                  <div class="">
                     <a class=" logo-biss" href="index.php"> <img src="assets/images/logo_img.png"></a>
                  </div>
                  <p class="footer-h">Discover seamless and convenient transportation with our online tricycle dispatching service! Whether you're running errands or exploring the city, our reliable tricycles are just a click away. Enjoy quick booking, affordable rates, and exceptional service at your fingertips!</p>
                 
               </div>
               <div class="col-md-1 ">
               </div>
               <div class="col-md-5">
                  <h2 class="footer-top">PHILIPPINE ORGANIZATIONS</h2>
                  <hr>
                  <ul class="footer-menu">
                     <li><a href="">• Department of Transportation (DOTr) </a></li>
                     <li><a href="">• Land Transportation Office (LTO) </a></li>
                     <li><a href="">• Land Transportation Franchising and Regulatory Board (LTFRB) </a></li>
                     <li><a href="">• Metropolitan Manila Development Authority (MMDA) </a></li>
                     <li><a href="">• Department of Public Works and Highways (DPWH) </a></li>
                     <li><a href="">• Local Government Units (LGUs) </a></li>
                  </ul>
               </div>


            </div>
         </div>
      </div>
   </div>